    ...
    const fetchData = async (e) => {
        try {
            ...
            for (var i = 0; i < result.length; i++) {
                const vals = result[i];
                if (e.target.value === '11' && vals[7] == 'Deployed') {
                    console.log(vals[7]);
                    const row = [vals[13].toString(), vals[1], vals[2], vals[3], vals[4], timestampToDate(vals[9])];
                    newData.push(row);
                } else if (e.target.value === '22' && vals[6] === 'Ready to Deploy') {
                    console.log(vals[7]);
                    const row = [vals[13], vals[1], vals[2], vals[3], vals[4], timestampToDate(vals[9])];
                    newData.push(row);
                }
            }
            setData(newData);
            ...
            try {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    if (vals[7] === 'Ready to Deploy') {
                        const row = [vals[12].toString(), vals[1], vals[2], vals[3], vals[4], timestampToDate(vals[9])];
                        newData.push(row);
                    }
                }
                setData(newData);
