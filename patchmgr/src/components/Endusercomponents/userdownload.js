...
const Userdownload = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const ids = await window.contract.methods.deployedarray().call();
                const newData = [];

                for (let i = 0; i < ids.length; i++) {
                    const vals = await window.contract.methods.details(ids[i]).call();
                    const row = [i + 1, vals[0], vals[1], vals[2], vals[3], timestampToDate(vals[8])];
                    newData.push(row);
                }
    ...
    const func1 = async (pn) => {
        ...
        await axios.post('http://localhost:5000/api/downloads', { fname: pn,uname: user });
    }
                    ...
                    <tbody>
                        {data.map((row, i) => (
                            <tr key={i}>
                                ...
                                <td>{row[4]}</td>
                                <td>{row[5]}</td>
                                <td>{<button className='btn btn-primary' onClick={() => func1(row[1])}>DOWNLOAD  <Download/></button>}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
